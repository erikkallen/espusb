...
static uint32_t bytes_rd = 0;
static uint32_t bytes_wr = 0;
static uint32_t reads = 0;
static uint32_t writes = 0;
static uint32_t error_after_bytes_written = 0;
static uint32_t error_after_bytes_read = 0;
...
static uint32_t fs_check_fixes = 0;
...
void hexdump_mem(uint8_t *b, uint32_t len) {
  while (len--) {
...
void area_set(uint32_t addr, uint8_t d, uint32_t size) {
  int i;
  for (i = 0; i < size; i++) {
    AREA(addr + i) = d;
  }
...
void fs_set_addr_offset(uint32_t offset) {
  addr_offset = offset;
}
...
uint32_t get_flash_ops_log_read_bytes() {
  return bytes_rd;
}

uint32_t get_flash_ops_log_write_bytes() {
  return bytes_wr;
}

void invoke_error_after_read_bytes(uint32_t b, char once_only) {
  error_after_bytes_read = b;
  error_after_bytes_read_once_only = once_only;
}
void invoke_error_after_write_bytes(uint32_t b, char once_only) {
...
void memrand(uint8_t *b, int len) {
  int i;
  for (i = 0; i < len; i++) {
    b[i] = rand();
  }
...
static uint32_t chits_tot = 0;
static uint32_t cmiss_tot = 0;
#endif
#endif
