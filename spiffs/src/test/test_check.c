...
TEST(evil_write) {
  ...
  uint32_t data_range = SPIFFS_CFG_LOG_BLOCK_SZ(FS) -
      SPIFFS_CFG_LOG_PAGE_SZ(FS) * (SPIFFS_OBJ_LOOKUP_PAGES(FS));
  uint8_t *corruption = malloc(data_range);
  memrand(corruption, data_range);
  uint32_t addr = 0 * SPIFFS_CFG_LOG_PAGE_SZ(FS) * SPIFFS_OBJ_LOOKUP_PAGES(FS);
...
TEST(lu_check1) {
  ...
  uint32_t addr = SPIFFS_BLOCK_TO_PADDR(FS, bix) + entry*sizeof(spiffs_obj_id);

  area_write(addr, (uint8_t*)&obj_id, sizeof(spiffs_obj_id));
...
TEST(page_cons1) {
  ...
  uint32_t addr = SPIFFS_PAGE_TO_PADDR(FS, pix) + sizeof(spiffs_page_object_ix_header) + 0 * sizeof(spiffs_page_ix);
  spiffs_page_ix bad_pix_ref = 0x55;
  area_write(addr, (uint8_t*)&bad_pix_ref, sizeof(spiffs_page_ix));
  area_write(addr+2, (uint8_t*)&bad_pix_ref, sizeof(spiffs_page_ix));
...
TEST(page_cons2) {
  ...
  uint32_t addr = SPIFFS_PAGE_TO_PADDR(FS, pix) + sizeof(spiffs_page_object_ix_header) + 1 * sizeof(spiffs_page_ix);
  spiffs_page_ix bad_pix_ref = dpix;
  area_write(addr, (uint8_t*)&bad_pix_ref, sizeof(spiffs_page_ix));
  area_write(addr+sizeof(spiffs_page_ix), (uint8_t*)&bad_pix_ref, sizeof(spiffs_page_ix));
...
TEST(page_cons_final) {
  ...
  uint32_t addr = SPIFFS_PAGE_TO_PADDR(FS, pix) + offsetof(spiffs_page_header, flags);
  uint8_t flags;
  area_read(addr, (uint8_t*)&flags, 1);
  flags |= SPIFFS_PH_FLAG_FINAL;
  area_write(addr, (uint8_t*)&flags, 1);
...
TEST(index_cons4) {
  ...
  uint32_t addr = SPIFFS_PAGE_TO_PADDR(FS, pix) + offsetof(spiffs_page_header, flags);
  uint8_t flags = 0xff & ~(SPIFFS_PH_FLAG_FINAL | SPIFFS_PH_FLAG_USED | SPIFFS_PH_FLAG_INDEX | SPIFFS_PH_FLAG_IXDELE);
