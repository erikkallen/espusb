...
TEST(read_beyond)
{
  ...
  spiffs_file fd;
  int32_t res;
  uint32_t size = SPIFFS_DATA_PAGE_SIZE(FS)*2;
...
TEST(bad_index_1) {
  ...
  uint32_t addr = SPIFFS_PAGE_TO_PADDR(FS, pix) + sizeof(spiffs_page_object_ix_header) + 2 * sizeof(spiffs_page_ix);
...
TEST(lseek_read) {
  ...
  while (runs--) {
    ...
    res = SPIFFS_lseek(FS, fd, -((uint32_t)sizeof(buf)+11), SPIFFS_SEEK_CUR);
    TEST_CHECK(res >= 0);
    offs -= (sizeof(buf)+11);
    res = SPIFFS_read(FS, fd, buf, sizeof(buf));
    TEST_CHECK(res >= 0);
...
TEST(long_run)
{
  ...
  int macro_runs = 500;
  printf("  ");
  uint32_t clob_size = SPIFFS_CFG_PHYS_SZ(FS)/4;
