...
int32_t SPIFFS_rename(spiffs *fs, const char *old, const char *new) {
#if SPIFFS_READ_ONLY
  ...
  int32_t res = spiffs_object_find_object_index_header_by_name(fs, (const uint8_t*)old, &pix_old);
  SPIFFS_API_CHECK_RES_UNLOCK(fs, res);

  res = spiffs_object_find_object_index_header_by_name(fs, (const uint8_t*)new, &pix_dummy);
  ...
  res = spiffs_object_update_index_hdr(fs, fd, fd->obj_id, fd->objix_hdr_pix, 0, (const uint8_t*)new,
      0, &pix_dummy);
